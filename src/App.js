...
function MemeBlazer() {
  ...
  const burnToken = async () => {
    if (!selectedToken || !wallet.connected) return;
    setIsLoading(true);
    ...
    try {
      ...
      const token = new Token(connection, tokenMint, TOKEN_PROGRAM_ID, wallet.publicKey);
      ...
      triggerConfetti();
      setTokens(tokens.filter(t => t.mint !== selectedToken.mint));
